...
function DesktopView({ openWindow, isDark }: { openWindow: (id: AppId) => void; isDark: boolean }) {
  return (
    <div className="absolute inset-0 top-10 bottom-16 flex items-center justify-center p-8 gap-8">
      {/* Left: Welcome Hero */}
      <motion.div
        initial={{ opacity: 0, x: -30 }}
        animate={{ opacity: 1, x: 0 }}
        transition={{ delay: 0.2, duration: 0.6 }}
        className={`p-10 max-w-lg w-full holographic ${isDark ? "liquid-glass-dark" : "liquid-glass"}`}
      >
        ...
        <div className="mb-2">
          ...
          <FlipLink
            ...
            gradient="linear-gradient(135deg, #3B82F6 0%, #8B5CF6 40%, #6366F1 100%)"
          >
            Swaroop
          </FlipLink>
        </div>
      ...
      {/* Right: Widget Grid — two columns */}
      <motion.div
        initial={{ opacity: 0, x: 30 }}
        animate={{ opacity: 1, x: 0 }}
        transition={{ delay: 0.3, duration: 0.6 }}
        className="flex gap-3 max-h-[calc(100vh-120px)]"
      >
