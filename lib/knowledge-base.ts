...
## PORTFOLIO WEBSITE
"SwaroopOS" — a desktop-OS-inspired portfolio built with Next.js 16, React 19, TypeScript, Tailwind v4, and Framer Motion. Entering it feels like unlocking an OS: the home route opens on a lock screen (name, title, live clock) and one swipe up — or any click or key press — reveals the desktop with proof points and the recruiter fast-path; reduced-motion users pass straight through, and deep links (/projects, /projects/alia, /projects/hrms, /resume, /contact) are never gated. Spotlight search (Cmd/Ctrl+K), draggable windows, terminal with real project data, this assistant, resume view + download. Live at swaroopos.vercel.app. He designed and built it himself — it is itself a proof of work. This assistant is static-data grounded (not RAG — no retrieval step).
`;
