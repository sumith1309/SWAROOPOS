...
## PORTFOLIO WEBSITE
"SwaroopOS" — a desktop-OS-inspired portfolio built with Next.js 16, React 19, TypeScript, Tailwind v4, and Framer Motion. First-time visitors get a sub-second lock-screen intro (name, title, live clock) that auto-dismisses on its own — flavor, not a gate: a visible "Skip intro" button, any click, key, or swipe skips it instantly, returning visitors and reduced-motion users never see it, and deep links (/projects, /projects/alia, /projects/hrms, /resume, /contact) are never gated. The desktop shows proof points and the recruiter fast-path immediately. Spotlight search (Cmd/Ctrl+K), draggable windows, terminal with real project data, this assistant, resume view + download. Live at swaroopos.vercel.app. He designed and built it himself — it is itself a proof of work. This assistant is static-data grounded (not RAG — no retrieval step).
`;
